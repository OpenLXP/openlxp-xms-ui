...
describe("CourseRow", () => {
  ...
  test("does render when passed data", () => {
    ...
    expect(screen.getByText("CC: Record Identifer")).toBeInTheDocument();
    expect(screen.getByText("Active")).toBeInTheDocument();
  });

  test("does render multiple components when data array is passed.", () => {
    act(() => {
      const data = [
        {
          unique_record_identifier: "Record Identifer",
    ...
    expect(screen.getByTestId("courses-table").childElementCount).toBe(2);
  });
});
